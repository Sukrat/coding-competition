...
function solve(str: string): number {
    ...
    let ingredientsHasAllergen = new Set<string>()
    ...
    ingredientsHasAllergen = new Set<string>(Array.from(map.values()).flatMap(m => Array.from(m)))
    const dontHaveAllergen = menu.flatMap(([ing, all]) => ing)
        .filter(m => !ingredientsHasAllergen.has(m))

    return dontHaveAllergen.length
}
